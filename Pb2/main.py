...
app = FastAPI(title="DAIYAN ULTRA SPAM - Dashboard")
...
def load_users():
    if not os.path.exists(USERS_FILE):
        default_users = {
            "daiyan": {"password": "father", "role": "admin"}
        }
    ...
    try:
        ...
        return {"daiyan": {"password": "daiyanspam", "role": "admin"}}
...
# =================== ULTRA FAST - ZERO REST SPAM ENGINE ===================
async def run_unlimited_spam(username: str, accounts, target_uid: str, region: str, badges_str: str, fast_mode: bool):
    ...
    add_user_log(username, f"🚀 ULTRA FAST START: {target_uid} | Region: {region}", "success")

    # 🔥 ZERO REST - CONTINUOUS ATTACK
    account_index = 0
    total_accounts = len(accounts)
    
    if total_accounts == 0:
        add_user_log(username, "❌ No accounts found!", "error")
        return

    # 🔥 Pre-load connections - continuous flow
    active_tasks = []
    MAX_CONCURRENT = 30  # Maximum parallel attacks
    ...
    while target_data["is_running"] and not target_data["stop_requested"]:
        ...
        # 🔥 Keep adding tasks continuously - NO REST
        for _ in range(5):  # Add 5 new tasks every cycle
            if not target_data["is_running"] or target_data["stop_requested"]:
                break
            if len(active_tasks) >= MAX_CONCURRENT:
                break
                
            bot_uid, password = accounts[account_index % total_accounts]
            account_index += 1
            
            task = asyncio.create_task(
                process_ultra_fast(username, target_data, bot_uid, password, 
                                 target_uid, region, badges_str, fast_mode)
            )
            active_tasks.append(task)

        # 🔥 Clean finished tasks
        if active_tasks:
            done, pending = await asyncio.wait(active_tasks, timeout=0.01, 
...
async def process_ultra_fast(username, target_data, bot_uid, password, target_uid, region, badges_str, fast_mode):
    """ULTRA FAST - Zero Rest, Maximum Speed"""
    try:
        # 🔥 SUPER FAST LOGIN
    ...
    users.forEach(u => {
      listCont.innerHTML += `
        <div style="background: rgba(255,255,255,0.05); padding: 5px 10px; border-radius:6px; display:flex; align-items:center; gap:10px; border:1px solid rgba(255,255,255,0.1)">
          ...
          ${u.username !== "daiyan" ? `<button style="background:none; border:none; color:red; cursor:pointer;" onclick="deleteUser('${u.username}')">❌</button>` : ''}
        </div>
      `;
    });
  }
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("🔥 DAIYAN ULTRA SPAM SYSTEM STABLE")
    print("🚀 Auto Multi-User Mode Activated")
    print("📡 Default Admin -> User: daiyan | Pass: father")
    print("="*60 + "\n")
